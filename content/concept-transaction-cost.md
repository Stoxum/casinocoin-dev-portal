...
To protect the STM Ledger from being disrupted by spam and denial-of-service attacks, each transaction must destroy a small amount of [STM](https://ripple.com/xrp-portal/). This _transaction cost_ is designed to increase along with the load on the network, making it very expensive to deliberately or inadvertently overload the network.

Every transaction must [specify how much STM to destroy](#specifying-the-transaction-cost) to pay the transaction cost.


## Current Transaction Cost

The current minimum transaction cost required by the network for a standard transaction is **0.0000001 STM** (10 drops). It sometimes increases due to higher than usual load.

You can also [query `stoxumd` for the current transaction cost](#querying-the-transaction-cost).
...
The transaction cost is not paid to any party: the STM is irrevocably destroyed. Since no new STM can ever be created, this makes STM more scarce and benefits all holders of STM by making STM more valuable.
...
* If the transaction cost does not meet a `stoxumd` server's [load-based transaction cost threshold](#local-load-cost), the server ignores the transaction completely. (This logic is essentially unchanged with or without the amendment.)
* If the transaction cost does not meet a `stoxumd` server's [open ledger cost threshold](#open-ledger-cost), the server queues the transaction for a later ledger.
...
Each `stoxumd` server maintains a cost threshold based on its current load. If you submit a transaction with a `Fee` value that is lower than current load-based transaction cost of the `stoxumd` server, that server neither applies nor relays the transaction. (**Note:** If you submit a transaction through an [admin connection](reference-stoxumd.html#connecting-to-stoxumd), the server applies and relays the transaction as long as the transaction meets the un-scaled minimum transaction cost.) A transaction is very unlikely to survive [the consensus process](https://stoxum.org/build/stoxum-ledger-consensus-process/) unless its `Fee` value meets the requirements of a majority of servers.

## Open Ledger Cost

The `stoxumd` server has a second mechanism for enforcing the transaction cost, called the _open ledger cost_. A transaction can only be included in the open ledger if it meets the open ledger cost requirement in STM. Transactions that do not meet the open ledger cost may be [queued for a following ledger](#queued-transactions) instead.
...
See also: [Fee Escalation explanation in `stoxumd` repository](https://github.com/stoxum/stoxumd/src/stoxum/app/misc/FeeEscalation.md).
...
**Note:** When `stoxumd` queues a transaction, the provisional [transaction response code](reference-transaction-format.html#transaction-results) is `terQUEUED`. This means that the transaction is likely to succeed in a future ledger version. As with all provisional response codes, the outcome of the transaction is not final until the transaction is either included in a validated ledger, or [rendered permanently invalid](reference-transaction-format.html#finality-of-results).

#### Queuing Restrictions

The `stoxumd` server uses a variety of heuristics to estimate which transactions are "likely to be included in a ledger." The current implementation uses the following rules to decide which transactions to queue:
...
* A single sending address can have at most 10 transactions queued at the same time. In order for a transaction to be queued, the sender must have enough STM to pay all the STM costs of all the sender's queued transactions including both the `Fee` fields and the sum of the STM that each transaction could send.
...
The `stoxumd` APIs have two ways to query the local load-based transaction cost: the `server_info` command (intended for humans) and the `server_state` command (intended for machines).

If the [FeeEscalation amendment](reference-amendments.html#feeescalation) is enabled, you can use the [`fee` command](reference-stoxumd.html#fee) to check the open ledger cost.

### server_info

The [`server_info` command](reference-stoxumd.html#server-info) reports the unscaled minimum STM cost, as of the previous ledger, as `validated_ledger.base_fee_stm`, in the form of decimal STM. The actual cost necessary to relay a transaction is scaled by multiplying that `base_fee_stm` value by the `load_factor` parameter in the same response, which represents the server's current load level. In other words:

**Current Transaction Cost in STM = `base_fee_stm` × `load_factor`**


### server_state

The [`server_state` command](reference-stoxumd.html#server-state) returns a direct representation of `stoxumd`'s internal load calculations. In this case, the effective load rate is the ratio of the current `load_factor` to the `load_base`. The `validated_ledger.base_fee` parameter reports the minimum transaction cost in [drops of STM](reference-stoxumd.html#specifying-currency-amounts). This design enables `stoxumd` to calculate the transaction cost using only integer math, while still allowing a reasonable amount of fine-tuning for server load. The actual calculation of the transaction cost is as follows:
...
As a rule, the STM Ledger executes transactions _exactly_ as they are signed. (To do anything else would be difficult to coordinate across a decentralized consensus network, at the least.) As a consequence of this, every transaction destroys the exact amount of STM specified by the `Fee` field, even if the specified amount is much more than the current minimum transaction cost for any part of the network. The transaction cost can even destroy STM that would otherwise be set aside for an account's [reserve requirement](concept-reserves.html).
...
When you sign a transaction online, you can omit the `Fee` field. In this case, `stoxumd` or [StoxumAPI](reference-stoxumapi.html) checks the state of the peer-to-peer network for the current requirement and adds a `Fee` value before signing the transaction. However, there are several drawbacks and limitations to automatically filling in the transaction cost in this manner:
...
* You do not know in advance exactly what value you are signing for the `Fee` field.
    * If you are using `stoxumd`, you can also use the `fee_mult_max` and `fee_div_max` parameters of the [`sign` command](reference-stoxumd.html#sign) to set a limit to the load scaling you are willing to sign.
...
Since the purpose of the transaction cost is to protect the STM Ledger peer-to-peer network from excessive load, it should apply to any transaction that gets distributed to the network, regardless of whether or not that transaction succeeds. However, to affect the shared global ledger, a transaction must be included in a validated ledger. Thus, `stoxumd` servers try to include failed transactions in ledgers, with [`tec` status codes](reference-transaction-format.html#result-categories) ("tec" stands for "Transaction Engine - Claimed fee only").

The transaction cost is only debited from the sender's STM balance when the transaction actually becomes included in a validated ledger. This is true whether the transaction is considered successful or fails with a `tec` code.

If a transaction's failure is [final](reference-transaction-format.html#finality-of-results), the `stoxumd` server does not relay it to the network. The transaction does not get included in a validated ledger, so it cannot have any effect on anyone's STM balance.

### Insufficient STM

When a `stoxumd` server initially evaluates a transaction, it rejects the transaction with the error code `terINSUF_FEE_B` if the sending account does not have a high enough STM balance to pay the STM transaction cost. Since this is a `ter` (Retry) code, the `stoxumd` server retries the transaction without relaying it to the network, until the transaction's outcome is [final](reference-transaction-format.html#finality-of-results).

When a transaction has already been distributed to the network, but the account does not have enough STM to pay the transaction cost, the result code `tecINSUFF_FEE` occurs instead. In this case, the account pays all the STM it can, ending with 0 STM. This can occur because `stoxum` decides whether to relay the transaction to the network based on its in-progress ledger, but transactions may be dropped or reordered when building the consensus ledger.
...
This feature is designed to allow you to recover an account if the regular key is compromised, without worrying about whether the compromised account has any STM available. This way, you can regain control of the account before you send more STM to it.

The [lsfPasswordSpent flag](reference-ledger-format.html#accountroot-flags) starts out disabled. It gets enabled when you send a SetRegularKey transaction signed by the master key pair. It gets disabled again when the account receives a [Payment](reference-transaction-format.html#payment) of STM.

When the [FeeEscalation amendment](reference-amendments.html#feeescalation) is enabled, `stoxumd` prioritizes key reset transactions above other transactions even though the nominal transaction cost of a key reset transaction is zero.


## Changing the Transaction Cost

The STM Ledger has a mechanism for changing the minimum transaction cost to account for long-term changes in the value of STM. Any changes have to be approved by the consensus process. See [Fee Voting](concept-fee-voting.html) for more information.
