...
A _Transaction_ is the only way to modify the STM Ledger. Transactions are only valid if signed, submitted, and accepted into a validated ledger version following the [consensus process](https://stoxum.org/build/stoxum-ledger-consensus-process/). Some ledger rules also generate _[pseudo-transactions](#pseudo-transactions)_, which aren't signed or submitted, but still must be accepted by consensus. Transactions that fail are also included in ledgers because they modify balances of STM to pay for the anti-spam [transaction cost](concept-transaction-cost.html).
...
In the decentralized STM Ledger, a digital signature proves that a transaction is authorized to do a specific set of actions. Only signed transactions can be submitted to the network and included in a validated ledger. A signed transaction is immutable: its contents cannot change, and the signature is not valid for any other transaction. <!-- STYLE_OVERRIDE: is authorized to -->
...
Sending a transaction to the STM Ledger involves several steps:
...
3. Submit a transaction to a `stoxumd` server. If the transaction is properly formed, the server provisionally applies the transaction to its current version of the ledger and relays the transaction to other members of the peer-to-peer network.
4. The [consensus process](https://stoxum.org/build/stoxum-ledger-consensus-process/) determines which provisional transactions get included in the next validated ledger.
5. The `stoxumd` servers apply those transactions to the previous ledger in a canonical order and share their results.
6. If enough [trusted validators](tutorial-stoxumd-setup.html#reasons-to-run-a-validator) created the exact same ledger, that ledger is declared _validated_ and the [results of the transactions](#transaction-results) in that ledger are immutable.
...
The STM Ledger only relays and executes a transaction if the transaction object has been authorized by the sending address (in the `Account`) field. For transactions authorized by only a single signature, you have two options:
...
1. Convert it to a binary blob and sign it offline. This is preferable, since it means that the account secret used for signing the transaction is never transmitted over any network connection.
    * You can use [StoxumAPI](reference-stoxumapi.html#sign) for offline signing.
2. Have a `stoxumd` server sign the transaction for you. The [sign command](reference-stoxumd.html#sign) takes a JSON-format transaction and secret and returns the signed binary transaction format ready for submission. (Transmitting your account secret is dangerous, so you should only do this from within a trusted and encrypted connection, or through a local connection, and only to a server you control.)
    * As a shortcut, you can use the [submit command](reference-stoxumd.html#submit) with a `tx_json` object to sign and submit a transaction all at once. This is only recommended for testing and development purposes.

In either case, signing a transaction generates a binary blob that can be submitted to the network. This means using `stoxumd`'s [submit command](reference-stoxumd.html#submit). Here is an example of the same transaction, as a signed blob, being submitted with the WebSocket API:
...
After a transaction has been submitted, you can check its status using the API, for example using the [tx command](reference-stoxumd.html#tx).
...
Multi-signing in the STM Ledger is the act of [authorizing transactions](#authorizing-transactions) for the STM Ledger by using a combination of multiple secret keys. You can have any combination of authorization methods enabled for your address, including multi-signing, a master key, and a [regular key](#setregularkey). (The only requirement is that _at least one_ method must be enabled.)
...
* If presented in binary form, the `Signers` array must be sorted based on the numeric value of the signer addresses, with the lowest value first. (If submitted as JSON, the [`submit_multisigned` command](reference-stoxumd.html#submit-multisigned) handles this automatically.)
...
The `"hash"` is the unique value that identifies a particular transaction. The server provides the hash in the response when you submit the transaction; you can also look up a transaction in an account's transaction history with the [account_tx command](reference-stoxumd.html#account-tx).
...
| [Fee][]                | String           | Amount            | (Required, but [auto-fillable](#auto-fillable-fields)) Integer amount of STM, in drops, to be destroyed as a cost for distributing this transaction to the network. |
...
Some fields can be automatically filled in before the transaction is signed, either by a `stoxumd` server or by the library used for offline signing. Both [stoxum-libjs](https://github.com/stoxum/stoxum-libjs) and `stoxumd` can automatically provide the following values:

* `Fee` - Automatically fill in the [transaction cost](concept-transaction-cost.html) based on the network. (*Note:* `stoxumd`'s [sign command](reference-stoxumd.html#sign) supports limits on how high the filled-in-value is, using the `fee_mult_max` parameter.)
...
To protect the STM Ledger from being disrupted by spam and denial-of-service attacks, each transaction must destroy a small amount of STM. This _[transaction cost](concept-transaction-cost.html)_ is designed to increase along with the load on the network, making it very expensive to deliberately or inadvertently overload the network.

The `Fee` field specifies an amount, in [drops of STM][Currency Amount], to destroy as the cost for relaying this transaction. If the transaction is included in a validated ledger (whether or not it achieves its intended purpose), then the amount of STM specified in the `Fee` parameter is destroyed forever. You can [look up the transaction cost](concept-transaction-cost.html#querying-the-transaction-cost) in advance, or [let `stoxumd` set it automatically](concept-transaction-cost.html#automatically-specifying-the-transaction-cost) when you sign a transaction.
...
An important and intentional feature of the STM Ledger is that a transaction is final as soon as it has been incorporated in a validated ledger.
...
We strongly recommend that you specify the `LastLedgerSequence` parameter on every transaction. Provide a value of about 3 higher than [the most recent ledger index](reference-stoxumd.html#ledger) to ensure that your transaction is either validated or rejected within a matter of seconds.
...
* [EscrowCancel - Reclaim escrowed STM](#escrowcancel)
* [EscrowCreate - Create an escrowed STM payment](#escrowcreate)
* [EscrowFinish - Deliver escrowed STM to recipient](#escrowfinish)
...
* [PaymentChannelFund - Add more STM to a payment channel](#paymentchannelfund)
...
The response from the [`submit` command](reference-stoxumd.html#submit) contains a provisional result from the `stoxumd` server indicating what happened during local processing of the transaction.
...
To see the final result of a transaction, use the [`tx` command](reference-stoxumd.html#tx), [`account_tx` command](reference-stoxumd.html#account-tx), or other response from `stoxumd`. Look for `"validated": true` to indicate that this response uses a ledger version that has been validated by consensus.
...
|:----------------------|:--------------------|:-------------------------------|
| Local error           | [tel](#tel-codes)   | The stoxumd server had an error due to local conditions, such as high load. You may get a different response if you resubmit to a different server or at a different time. |
...
[[Source]<br>](https://github.com/stoxum/stoxumd/src/stoxum/protocol/TER.h "Source")
...
| `telNO_DST`_`PARTIAL`   | The transaction is an STM payment that would fund a new account, but the [tfPartialPayment flag](#partial-payments) was enabled. This is disallowed. |

### tem Codes

These codes indicate that the transaction was malformed, and cannot succeed according to the STM Ledger protocol. They have numerical values in the range -299 to -200. The exact code for any given error is subject to change, so don't rely on it.
...
| `temBAD_FEE`                  | The transaction improperly specified its `Fee` value, for example by listing a non-STM currency or some negative amount of STM. |
...
| `temBAD_OFFER`                | The [OfferCreate transaction][] specifies an invalid offer, such as offering to trade STM for itself, or offering a negative amount. |
| `temBAD_PATH`                 | The [Payment](#payment) transaction specifies one or more [Paths](#paths) improperly, for example including an issuer for STM, or specifying an account differently. |
...
| `temBAD_SEND_STM_LIMIT`     | The [Payment](#payment) transaction used the [tfLimitQuality](#limit-quality) flag in a direct STM-to-STM payment, even though STM-to-STM payments do not involve any conversions. |
| `temBAD_SEND_STM_MAX`       | The [Payment](#payment) transaction included a `SendMax` field in a direct STM-to-STM payment, even though sending STM should never require SendMax. (STM is only valid in SendMax if the destination `Amount` is not STM.) |
| `temBAD_SEND_STM_NO_DIRECT` | The [Payment](#payment) transaction used the [tfNoDirectStoxum](#payment-flags) flag for a direct STM-to-STM payment, even though STM-to-STM payments are always direct. |
| `temBAD_SEND_STM_PARTIAL`   | The [Payment](#payment) transaction used the [tfPartialPayment](#partial-payments) flag for a direct STM-to-STM payment, even though STM-to-STM payments should always deliver the full amount. |
| `temBAD_SEND_STM_PATHS`     | The [Payment](#payment) transaction included `Paths` while sending STM, even though STM-to-STM payments should always be direct. |
...
| `tefBAD_LEDGER`         | While processing the transaction, the ledger was discovered in an unexpected state. If you can reproduce this error, please [report an issue](https://github.com/stoxum/stoxumd/issues) to get it fixed. |
...
| `tefEXCEPTION`           | While processing the transaction, the server entered an unexpected state. This may be caused by unexpected inputs, for example if the binary data for the transaction is grossly malformed. If you can reproduce this error, please [report an issue](https://github.com/stoxum/stoxumd/issues) to get it fixed. |
| `tefFAILURE`             | Unspecified failure in applying the transaction.    |
| `tefINTERNAL`            | When trying to apply the transaction, the server entered an unexpected state. If you can reproduce this error, please [report an issue](https://github.com/stoxum/stoxumd/issues) to get it fixed. |
| `tefINVARIANT_FAILED`   | An invariant check failed when trying to claim the [transaction cost](concept-transaction-cost.html). Requires the [EnforceInvariants amendment](reference-amendments.html#enforceinvariants). If you can reproduce this error, please [report an issue](https://github.com/stoxum/stoxumd/issues). |
...
| `terINSUF_FEE_B` | The account sending the transaction does not have enough STM to pay the `Fee` specified in the transaction. |
...
| `terno_stoxum`    | Used internally only. This code should never be returned. |
...
| `tecINSUF_RESERVE_LINE`    | 122   | The transaction failed because the sending account does not have enough STM to create a new trust line. (See: [Reserves](concept-reserves.html)) This error occurs when the counterparty already has a trust line in a non-default state to the sending account for the same currency. (See `tecNO_LINE_INSUF_RESERVE` for the other case.) |
| `tecINSUF_RESERVE_OFFER`   | 123   | The transaction failed because the sending account does not have enough STM to create a new Offer. (See: [Reserves](concept-reserves.html)) |
...
| `tecINTERNAL`              | 144   | Unspecified internal error, with transaction cost applied. This error code should not normally be returned. If you can reproduce this error, please [report an issue](https://github.com/stoxum/stoxumd/issues). |
| `tecINVARIANT_FAILED`      | 147   | An invariant check failed when trying to execute this transaction. Requires the [EnforceInvariants amendment](reference-amendments.html#enforceinvariants). If you can reproduce this error, please [report an issue](https://github.com/stoxum/stoxumd/issues). |
...
| `tecNO_ALTERNATIVE_KEY`    | 130   | The transaction tried to remove the only available method of [authorizing transactions](#authorizing-transactions). This could be a [SetRegularKey transaction][] to remove the regular key, a [SignerListSet transaction][] to delete a SignerList, or an [AccountSet transaction][] to disable the master key. (Prior to `stoxumd` 0.30.0, this was called `tecMASTER_DISABLED`.) |
...
| `tecNO_DST`                | 124   | The account on the receiving end of the transaction does not exist. This includes Payment and TrustSet transaction types. (It could be created if it received enough STM.) |
| `tecNO_DST_INSUF_STM`      | 125   | The account on the receiving end of the transaction does not exist, and the transaction is not sending enough STM to create it. |
...
| `tecNO_TARGET`             | 138   | The transaction referenced an Escrow or PayChannel ledger object that doesn't exist, either because it never existed or it has already been deleted. (For example, another [EscrowFinish transaction][] has already executed the held payment.) Alternatively, the destination account has `asfDisallowSTM` set so it cannot be the destination of this [PaymentChannelCreate][] or [EscrowCreate][] transaction. |
...
| `tecUNFUNDED`              | 129   | The transaction failed because the account does not hold enough STM to pay the amount in the transaction _and_ satisfy the additional reserve necessary to execute this transaction. (See: [Reserves](concept-reserves.html)) |
| `tecUNFUNDED_ADD`          | 102   | **DEPRECATED.**                         |
| `tecUNFUNDED_PAYMENT`      | 104   | The transaction failed because the sending account is trying to send more STM than it holds, not counting the reserve. (See: [Reserves](concept-reserves.html)) |
...
[Currency Amount]: reference-stoxumd.html#specifying-currency-amounts
